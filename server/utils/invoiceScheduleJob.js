...
import { calcSendDate, INVOICE_SEND_LEAD_DAYS } from './billingDate.js';
...
export const syncInvoiceSendSchedules = async () => {
  ...
  for (const s of schedules) {
    ...
    const next = (s.rounds || [])
      ...
      .map((r) => ({ round: r, sendDate: calcSendDate(r.dueDate) }))
  ...
  console.log(`[청구서 발송 일정] 출금일 ${INVOICE_SEND_LEAD_DAYS}일 전 기준 ${created}건 등록 (대상 없음 ${skipped}건)`);
